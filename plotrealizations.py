...
nfits = len(fitresults)
realids = numpy.floor(numpy.random.uniform(0, nfits, Nreal))

for ifit in range(Nreal):
    realid = numpy.int(realids[ifit])
    bestfit = fitresults[realid]
    ...
    for regioni in range(nregions):
        ...
        lnprob = bestfit['lnprob']
        objtitlelnprob = objtitle + ': ' + str(lnprob)
        plt.suptitle(objtitlelnprob, fontsize='x-large', y=0.99)
        ...
        itertag = '.iter' + str(realid)
        savefig(rename + '.sbmap.Region' + sri + itertag + '.pdf')
